...
class TurtleLoveNode(Node):
    
    def __init__(self):
        super().__init__("turtle_love")
        self.turtle_exists = False
        self.previous_y_pose_Cinderella = 0.0
        self.previous_y_pose_superman = 0.0
        self.spawn_service_condition()
        self.cmd_vel_publisher_Cinderella = self.create_publisher(Twist, '/Cinderella/cmd_vel', 10)
        self.cmd_vel_publisher_superman = self.create_publisher(Twist, '/Superman/cmd_vel', 10)
        self.pose_subscriber_Cinderella = self.create_subscription(Pose, "/Cinderella/pose", self.turtle_love_callback_Cinderella, 10)
        self.pose_subscriber_superman = self.create_subscription(Pose, "/Superman/pose", self.turtle_love_callback_superman,  10)
        self.get_logger().info("Turtles Cinderella and Superman have been spawned")

    def spawn_service_condition(self):
        if not self.turtle_exists:
            self.call_spawn_service(5.5, 2.75, 15.0, 'Cinderella')
            self.call_spawn_service(5.5, 2.75, 20.0, 'Superman')
            self.turtle_exists = True

    def turtle_love_callback_Cinderella(self, pose: Pose):
        cmd = Twist()
        if pose.y >= 8.25 and self.previous_y_pose_Cinderella < 8.25 and pose.x > 5.5:
            self.previous_y_pose_Cinderella = pose.y
            cmd.linear.x = 1.0
            cmd.angular.z = 0.9
        elif pose.y >= 8.25 and self.previous_y_pose_Cinderella < 8.25 and pose.x < 5.5:
            self.previous_y_pose_Cinderella = pose.y
            cmd.linear.x = 1.0
            cmd.angular.z = -0.9
        elif pose.y <= 8.25 and self.previous_y_pose_Cinderella > 8.25:
            self.previous_y_pose_Cinderella = pose.y
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
        elif pose.y < 8.25 and self.previous_y_pose_Cinderella < 8.25:
    ...
    def turtle_love_callback_superman(self, pose: Pose):
        cmd = Twist()
        if pose.y >= 8.25 and self.previous_y_pose_superman < 8.25 and pose.x > 5.5:
            self.previous_y_pose_superman = pose.y
            cmd.linear.x = 1.0
            cmd.angular.z = 0.9
        elif pose.y >= 8.25 and self.previous_y_pose_superman < 8.25 and pose.x < 5.5:
            self.previous_y_pose_superman = pose.y
            cmd.linear.x = 1.0
            cmd.angular.z = -0.9
        elif pose.y <= 8.25 and self.previous_y_pose_superman > 8.25:
            self.previous_y_pose_superman = pose.y
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
        elif pose.y < 8.25 and self.previous_y_pose_superman < 8.25:
